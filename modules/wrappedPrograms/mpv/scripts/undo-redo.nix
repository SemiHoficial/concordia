{
  ...
  # bandage fix, fix later
  perSystem = {pkgs, ...}: {
    packages.mpv-undo-redo = let
      mpvUndoRedoBinds = {
        undoBind = "ctrl+z";
        redoBind = "ctrl+shift+z";
      };
    in
      pkgs.stdenvNoCC.mkDerivation {
        name = "mpv-undo-redo";
        src =
          pkgs.writeText "undo-redo.lua"
          ''
            ...
            mp.add_key_binding("${mpvUndoRedoBinds.undoBind}", "undo", undo)

            mp.add_key_binding("${mpvUndoRedoBinds.redoBind}", "redo", redo)
        ...
        installPhase = ''
          ...
          cp $src $out/share/mpv/scripts/undo-redo.lua
        '';
        passthru.scriptName = "undo-redo.lua";
      };
  };
}
